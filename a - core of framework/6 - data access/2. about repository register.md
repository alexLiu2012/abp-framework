...
* 配置 abp 相关 dbContext_register 的 options
...
* 具体 repos register 继承 repos registrar base 方法
* `xxxRegistrar.AddRepositories()`是提供注册 repos 的方法
  * 需要派生类实现：
...
* 扩展 service.AddxxxDb() 方法
* 实际是
  * 配置相应的`xxxDbContextRegistrationOptions`
  * 调用 `xxxDbContextRegistrar.AddRepositories()` 方法
...
* 提供`AddRepositories()`方法（一般由 services 的扩展方法调用）

参考具体 db repos 实现

### 3. practice

#### 3.1 dbContext register options

##### 3.1.1 定义派生的 dbContext register options

* 继承 common dbContext register options

##### 3.1.2 配置派生的 dbContext register options

* 通过 configure_services 方法配置

* `AddxxxDbContext()`方法一般会调用 dbContext register 的`AddRepositories()`方法

  ```c#
  public override void ConfigureService(ServiceConfigurationContext context)
  {
      context.servcies.AddxxxDbContext(opitons =>
      	{
              // 配置 ...                              
          });
  }
...
#### 3.2 dbContext register

##### 3.2.1 定义派生的 dbContext register
