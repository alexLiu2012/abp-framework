...
###### 1.2.1.1 abp dbContext registration options

* ef core dbContext 的 registration options

* 继承 common dbContext register options

###### 1.2.1.2 efCore repo registrar

* 继承 repo register base
  * 实现 get entities
  * 实现 get repo type

###### 1.2.1.3 add efCoredb(Context)

* 扩展 services 的`AddAbpDbContext()`方法
  * 配置 efCore dbContext registration options
  * 调用 efCore repo registrar 的 addRepositories 方法

##### 1.2.2 efCore repo

* IRepo 在 efCore 下的实现
* 由 efCore 充当 proxy，连接不同的 database
* crud、event 等操作通过内部包含的：
  * DbContext、DbSet（ms ef core 组件）

* 上述组件由 abp dbContext 解析
  * abp dbContext 是 DbContext 的超集

##### 1.2.3 abp dbContext

###### 1.2.3.1 abp dbContext

* 解耦 repo 和 efCore
* 解析 efCore DbContext（其本身就是，超集，解析 = 类型转换）
* 定义数据库模型
  * 填充 efCore DbSet
  * 配置 efCore DbSet mapping

###### 1.2.3.2 IEfCoreDbContext

* efCore database 的概念，对应的 object model（数据库模型）
* 由 ms ef core 实现
* abp dbContext 是 IEfCoreDbContext 的超集，
  * 可以转换

###### 1.2.3.3 abp entity options

* efCore collection/table 的概念，对应的 object model（数据表模型）

##### 1.2.4 abp dbContext options

* dbContext 配置 action 的容器
* 封装 ms efCore DbContext options

##### 1.2.5 abp(efCore) dbContext provider

* 获取、创建 abp(efCore) dbContext provider
* 管理器
...
##### 2.1.1 abp dbContext register options

###### 2.1.1.1 abp dbContext registration options 接口
...
###### 2.1.1.2 abp dbContext registration options 实现
...
* 代表 entity model，即数据表 table / collection
...
public class AbpEntityOptions
{
        	...
        	where TEntity : IEntity
    {
        ...
        optionsAction(_options.GetOrAdd(
            typeof(TEntity),
	...
	RepositoryRegistrarBase<AbpDbContextRegistrationOptions>
{
    ...
    // 重写（实现）get entity type 方法，
    // 通过 (abp)DbContext helper 实现
    ...
    // 重写（实现）get repo type，
    // 为 entity 创建 efCore<tcontext,tentity>
    ...
    // 重写（实现）get repo type，
    // 为 entity 创建 efCore<tcontext,tentity,tkey>
    protected override Type GetRepositoryType(
        Type dbContextType, 
        Type entityType, 
        Type primaryKeyType)
...
* 工具类，获取 abp dbContext 中定义的需要注册 repo 的 entity 类型，
* 对于 efCore 来说，
  * 就是 DbSet<T> 的泛型类型 T
  * 并且实现 IEntity 接口  
...
public static class AbpEfCoreServiceCollectionExtensions
{
    public static IServiceCollection 
        AddAbpDbContext<TDbContext>(        
        	...
        	Action<IAbpDbContextRegistrationOptionsBuilder> 
        		optionsBuilder = null)        	
        where TDbContext : AbpDbContext<TDbContext>
    {
        services.AddMemoryCache();
                
        // 创建 abp dbContext registration options
        var options = new AbpDbContextRegistrationOptions(
            typeof(TDbContext), 
            services);
        // 配置 abp dbContext registration options
        ...
        // 注册 abp dbContext，
        // 并替换 options.ReplaceDbContextType
        ...
        // 创建 efCore repo registrar 并加载 repos
...
public class EfCoreRepository<TDbContext, TEntity> 
{
    ...
    {
        // 如果 entity options 中没有定义 details func
...
public class AbpEfCoreDbContextInitializationContext
{
    ...
    public AbpEfCoreDbContextInitializationContext(
...
###### 2.4.2.2 model builder db provider 扩展
...
public abstract class AbpDbContext<TDbContext> 
{
    ...
    protected virtual bool TryCancelDeletionForSoftDelete(
        EntityEntry entry)
    {
...
* 存储 abp dbContext options 配置的容器
...
public class AbpDbContextOptions
{
    ...
    // 以 TDbContext 类型为索引的 configuration
    ...
    // 添加 TDbContext 对应的 pre configuration
    ...
    // 设置 TDbContext 对应的 configuration
...
##### 2.5.1 dbContext configuration context

* 包裹 ms ef core DbContextOptionsBuilder 封装

###### 2.5.1.1 configuration context
...
###### 2.5.1.2 configuration context (T)
...
public class AbpDbContextConfigurationContext<TDbContext> : 
	AbpDbContextConfigurationContext    
        where TDbContext : AbpDbContext<TDbContext>
{
    // 创建 ms efCore dbContext options builder
...
##### 2.5.3 dbContext creation context
...
public class DbContextCreationContext
{
    ...
    public static DbContextCreationContext Current => 
        _current.Value;
    private static readonly AsyncLocal<DbContextCreationContext> 
        _current = new AsyncLocal<DbContextCreationContext>();
...
##### 2.5.4 dbContext options factory

* 通过 abp dbContext options 创建 (ms efCore) DbContext options
...
public static class DbContextOptionsFactory
{
    public static DbContextOptions<TDbContext> Create<TDbContext>(
        IServiceProvider serviceProvider)            
        	where TDbContext : AbpDbContext<TDbContext>
    {
        // 获取 (ms efCore) dbContext options creation context
        var creationContext = GetCreationContext<TDbContext>(serviceProvider);
                
        // 创建 abp dbContext configuration context
...
###### 2.5.4.1 get creation context

* 实际是创建了 dbContext creation context

```c#
public static class DbContextOptionsFactory
{
    private static DbContextCreationContext 
        GetCreationContext<TDbContext>(
        	IServiceProvider serviceProvider)            	
        where TDbContext : AbpDbContext<TDbContext>
    {
...
###### 2.5.4.2 get (abp) dbContext options

* 从 services 解析 abp dbContext options
* abp dbContext optinos 在 efCore module 中注册

```c#
public static class DbContextOptionsFactory
{
    private static AbpDbContextOptions 
        GetDbContextOptions<TDbContext>(
        	IServiceProvider serviceProvider)        
        where TDbContext : AbpDbContext<TDbContext>
    {
...
###### 2.5.4.3 pre-configure

* 执行 pre configuration
...
* 扩展 abp dbContext options 方法，
* 声明使用具体的 database，如同原生 ms efCore 一样
* 以 postgresql 为例

###### 2.5.5.1 abp dbContext options 扩展方法

```c#
// 通过 default configuration 添加 UsexxxDb 的配置
public static void UseNpgsql(    
    [NotNull] this AbpDbContextOptions options,            
    [CanBeNull] Action<NpgsqlDbContextOptionsBuilder> 
    	postgreSqlOptionsAction = null)
{
...
// 通过 TDbContext 类型对应的 configuration 添加 UsexxxDb 的配置
...
###### 2.5.5.2 abp dbContext configuration context 扩展方法

* 通过 configuration context 中封装的 ms efCore DbContext options 声明 UsexxxDb
    	...
    	postgreSqlOptionsAction = null)
{
    ...
    {
        return context.DbContextOptions
            .UseNpgsql(
...
public class UnitOfWorkDbContextProvider<TDbContext> 
{
    public TDbContext GetDbContext()
    {
        ...
        var connectionString = _connectionStringResolver
            .Resolve(connectionStringName);
        // 生成 dbContext key（没有使用）
        var dbContextKey = $"typeof(TDbContext).FullName}_{connectionString}";     
        
        // 获取 current uow 下的 efCore database api，
        // 如果没有，创建并添加
        ...
        // 返回 efCore database api 中的 dbContext
...
###### 2.6.2.3 create TDbContext by uow
...
###### 2.6.2.4 create transaction TDbContext by uow
...
###### 2.6.3.2 efCore transaction api
...
public class AbpEntityFrameworkCoreModule : AbpModule
{
        ...
        ServiceConfigurationContext context)
    {
        ...
        // 注册 abp(efCore) dbContext provider，        
...
* abp 扩展的匹配
  ...
  protected override void OnModelCreating(ModelBuilder builder)
  {
      ...
      builder.Entity<Book>(b =>
      {
          ...
          //(if you are using the fluent API)
