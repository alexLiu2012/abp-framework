...
* module loader 从 module container 中 load（加载）module container
* module manager 是对外（上层架构）提供模块加载、管理的接口
...
{
    // 是否进行自动注册
    protected internal bool SkipAutoServiceRegistration { get; protected set;}       // 注入 service configuration context 
...
* 自定义 module 继承 abp module，指定 depends
* 在模块中
  * 配置options
  * 注册服务
    * 注册拦截器
